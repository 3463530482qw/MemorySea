from pathlib import Path
import subprocess

build = Path(__file__).parent    #脚本目录
memorySea = build.parent.parent   #根目录

#检查模块数量是否为1
cpp_files = list(build.glob("*.cpp"))
if len(cpp_files) != 1:
    raise RuntimeError(f"需要唯一的 .cpp 文件，找到 {len(cpp_files)} 个: {[f.name for f in cpp_files]}")

main_cpp = cpp_files[0] #指定cpp文件
proj_name = main_cpp.stem #提取文件名字
build_dir = memorySea / "build" / "dll" / proj_name #拼接目标路径
build_dir.mkdir(parents=True, exist_ok=True) #创建路径

sources = [str(main_cpp)] #获取cpp文件路径
main_import_lib = memorySea / "build" / "libMemorySea.dll.a" #获取主程序导出的符号

include_args = [
    "-I", "D:/mingw64/SDL3-3.4.8/x86_64-w64-mingw32/include",
    "-I", "D:/mingw64/SDL3-3.4.8/x86_64-w64-mingw32/include/SDL3",
    "-I", "D:/mingw64/spine-runtimes-4.3/spine-cpp/include",
    "-I", "D:/mingw64/spine-runtimes-4.3/spine-sdl/src",
    "-I", "D:/mingw64/inih-r62/inih-r62",
    "-I", "D:/mingw64/simdjson-4.6.7/simdjson-4.6.7/include",
    "-I", "D:/mingw64/stb-master/stb-master",
    "-I", "D:/Luos",
    "-I", "C:/vulkan/Include",
    "-I", str(memorySea),
]

# 场景模块拆两步:先 ccache 编译 .o(命中缓存则秒过),再纯链接成 dll
obj = build_dir / f"{proj_name}.o"
subprocess.run([
    "ccache", "g++", "-std=c++23", "-Os",
    "-finput-charset=UTF-8", "-fexec-charset=UTF-8",
    "-DNDEBUG",
    "-DEXPORT=__declspec(dllexport)",
    "-c", str(main_cpp), "-o", str(obj),
    *include_args,
], check=True)

link_cmd = [
    "g++", "-shared", "-Os", "-s", "-std=c++23",
    "-DNDEBUG",
    "-o", str(build_dir / f"{proj_name}.dll"),
    str(obj),
    str(main_import_lib),
    "-L", "D:/mingw64/SDL3-3.4.8/x86_64-w64-mingw32/lib",
    "C:/vulkan/Lib/vulkan-1.lib",
    "-lSDL3",
    "-lwinmm"
]

subprocess.run(link_cmd, check=True)
print(f"{proj_name}.dll 生成在: {build_dir}")